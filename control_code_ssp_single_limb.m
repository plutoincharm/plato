...
hfx=BC(10,76) ;hfy=BC(11,76);  % knee
vhfx =  omg(10,76); vhfy = omg(11,76);% knee

xf = [thtf3;thtf4;hfx;hfy;omgf3;omgf4;vhfx;vhfy];
...
alp(4) =alpval(11,frame);   
 ...
 afall = [alp(1);alp(2);alp(3);alp(4)];
% first roll-out
for k = 2:Nt-1
         x(:,k) = dynamics_midpoint(x(:,k-1),u(:,k-1),dt)
        %time(k-1) = (k-1)*dt + 0.54 ;
               
        alp(1) =alpval(3,frame+k-1);   
        alp(2) =alpval(4,frame+k-1);   
        alp(3) =alpval(10,frame+k-1); 
        alp(4) =alpval(11,frame+k-1);   
...
grid on;
hold on;
plot(cr,alpval(10,20:76),'g-','LineWidth',1);
xlabel('time (sec) \rightarrow');
ylabel('Akx (N) \rightarrow');
legend('calc','dataset');
...
grid on;
hold on;
plot(cr,alpval(11,20:76),'g-','LineWidth',1);
xlabel('time (sec) \rightarrow');
ylabel('Aky (N) \rightarrow');
legend('calc','dataset');
