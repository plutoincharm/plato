...
global A B Nx Nu pert MI L m  nx ny tx ty g r lam vars misc alp alpval indic kc lamall xdata lamx lamy val af acal
...
x0 = [tht1;tht2;tht3;tht4;hx;hy;omg1;omg2;omg3;omg4;vhx;vhy];
...
xf = [thtf1;thtf2;thtf3;thtf4;hfx;hfy;omgf1;omgf2;omgf3;omgf4;vhfx;vhfy];
...
lamally = [lamy];
%alp = zeros(Nx/2);
alp(1) =alpval(1,frame);   
alp(2) =alpval(2,frame);   
alp(3) =alpval(3,frame); 
alp(4) =alpval(4,frame);   
alp(5) =alpval(8,frame);   
alp(6) =alpval(9,frame);  
%alp(4) = 0;     
 afall = [alp(1);alp(2);alp(3);alp(4);alp(5);alp(6)];
% first roll-out
for k = 2:Nt-1
        ...
        alp(1) =alpval(1,frame+k-1);   
        alp(2) =alpval(2,frame+k-1);   
        alp(3) =alpval(3,frame+k-1); 
        alp(4) =alpval(4,frame+k-1);   
        alp(5) =alpval(8,frame+k-1);   
        alp(6) =alpval(9,frame+k-1); 
 ...
 %%%%% Backward Pass %%%%%
    ...
    for k = (Nt-1):-1:1
          ...
          %Calculate derivatives of stage cost
           ...
           alp(1) =  alpval(4,frame+k-1);
           alp(2) =  alpval(10,frame+k-1);
           alp(3) =  alpval(11,frame+k-1);
...
%%%% End of Backward Pass %%%%%
   ...
   % alp =alpval(:,frame) 
    ...
    indic = 15;
    xdata = x0;   
     alp(1) =alpval(4,frame);   
    alp(2) =alpval(10,frame);   
    alp(3) =alpval(11,frame);   
   for kc = 1:(Nt-1)
        un(:,kc) = u(:,kc) - alpha*d(:,kc) - (K(:,:,kc)*(xn(:,kc)-x(:,kc)));
        xn(:,kc+1) = dynamics_midpoint(xn(:,kc),un(:,kc),dt);

        alp(1) =  alpval(4,frame+kc-1);
        alp(2) =  alpval(10,frame+kc-1);
        alp(3) =  alpval(11,frame+kc-1);
        %thetval = BC(:,frame+kc);
        %omgval = omg(:,frame+kc);
        %xdata = [thetval;omgval]; 
        %alp = zeros(Nx/2);
